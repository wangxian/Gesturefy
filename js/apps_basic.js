...
sue.apps = {
  ...
  initBox: function (boxInfo) {
    ...
    domBox = sue.apps.domCreate("smartup", {
      setName: ["className"],
      setValue: ["su_apps"]
    }, null, "z-index:" + (parseInt((new Date().getTime()) / 1000)), {
      setName: ["appname"],
      setValue: [boxInfo.appName]
    });
    //head
    domHead = sue.apps.domCreate("div", {
      setName: ["className"],
      setValue: ["su_head"]
    });
    if (boxInfo.headTitle) {
      let _domHeadTtitle = sue.apps.domCreate("span", {
        setName: ["className"],
        setValue: ["su_title"]
      }, null, null, null, sue.apps.i18n(boxInfo.headTitle));
    ...
    //main
    domMain = sue.apps.domCreate("div", {
      setName: ["className"],
      setValue: ["su_main"]
    });
    ...
    //menu
    if (boxInfo.menu) {
      domMenu = sue.apps.domCreate("div", {
        setName: ["className"],
        setValue: ["su_menu"]
      });
      for (var i = 0; i < boxInfo.menu.length; i++) {
        var _domMenuImg = sue.apps.domCreate("img", {
          setName: ["className"],
          setValue: [boxInfo.menu[i].className]
        }, null, null, boxInfo.menu[i].action ? ({
          setName: ["action"],
          setValue: [boxInfo.menu[i].action]
        }) : null);
    ...
    //options
    if (boxInfo.options) {
      domOptions = sue.apps.domCreate("div", {
        setName: ["className"],
        setValue: ["su_options"]
      });
      for (var i = 0; i < boxInfo.options.length; i++) {
        if (boxInfo.options[i].type == "select") {
          var _label = sue.apps.domCreate("label", {
            setName: ["className"],
            setValue: ["options_labelname"]
          }, null, null, null, sue.apps.i18n(boxInfo.options[i].name));
        ...
        if (boxInfo.options[i].type == "range") {
          var _label = sue.apps.domCreate("label", {
            setName: ["className"],
            setValue: ["options_labelname"]
          }, null, null, null, sue.apps.i18n(boxInfo.options[i].label));
          ...
          var _domSpan = sue.apps.domCreate("span", {
            setName: ["className"],
            setValue: ["options_rangebox"]
          });
      ...
      let _btnCancel = sue.apps.domCreate("input", {
        setName: ["className"],
        setValue: ["options_btn_cancel"]
      });
      _btnCancel.type = "button";
      _btnCancel.value = sue.apps.i18n("btn_cancel");
      let _btnSave = sue.apps.domCreate("input", {
        setName: ["className"],
        setValue: ["options_btn_save"]
      });
  ...
  showOpt: function (e) {
    ...
    if (_opt) {
      ...
      window.setTimeout(function () {
        domopt.style.cssText += "display:none;";
      }, 200)
    }
  },
  initPos: function (e) {
  ...
  notification: function (e, text, type, time) {
    ...
    let _domNotif = sue.apps.domCreate("div", {
      setName: ["className"],
      setValue: ["su_notifi"]
    }, null, null, null, text);
    switch (type) {
  ...
  showPanel: function (dom) {
    ...
    if (_show) {
      ...
      window.setTimeout(function () {
        dom.style.cssText += "opacity:0.96;z-index:10;";
      }, 10)
    } else {
