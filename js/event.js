    ...
    switch (e.type) {
      ...
      case "contextmenu":
        if (config.general.linux.cancelmenu &&
          ...
          sue.cons.fix_linux_timer = window.setTimeout(function () {
    ...
    if (confOBJ.tip) {
      if (config[sue.drawType[0]].ui.tip.withdir) {
        ...
        for (var i = 0; i < sue._dirArray.length; i++) {
          var _dir = sue.domCreate("img", {
            setName: ["src"],
            setValue: [chrome.extension.getURL("") + "image/direct.png"]
          }, null, "vertical-align: text-top;transform:rotate(+" + sue.directimg(sue._dirArray[i]) + ");height: " + config[sue.drawType[0]].ui.tip.width + "px;");
    ...
    if (confOBJ.allaction && confOBJ.allaction.length > 0) {
      for (var i = 0; i < confOBJ.allaction.length; i++) {
        ...
        for (var ii = 0; ii < confOBJ.allaction[i].direct.length; ii++) {
          var _img = sue.domCreate("img", {
            setName: ["src"],
            setValue: [chrome.extension.getURL("") + "image/direct.png"]
          }, null, "vertical-align: text-top;height:" + config[sue.drawType[0]].ui.allaction.width + "px;transform:rotate(" + sue.directimg(confOBJ.allaction[i].direct[ii]) + ");");
