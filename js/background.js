		...
		var config = {
			...
			mges: {
				...
				actions: [{
					...
					},
					{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_scroll_up")
						},
						selects: [{
							type: "n_scroll",
							value: "s_up"
					...
					},
					{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_scroll_down")
						},
						selects: [{
							type: "n_scroll",
							value: "s_down"
					...
					},
					{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_scroll_top")
						},
						selects: [{
							type: "n_scroll",
							value: "s_top"
					...
					},
					{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_scroll_bottom")
						},
						selects: [{
							type: "n_scroll",
							value: "s_bottom"
					...
					},
					{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_close_current")
						},
						selects: [{
								type: "n_tab",
								value: "s_current"
					...
					},
					{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_switch_left")
						},
						selects: [{
							type: "n_tab_lrhl",
							value: "s_left"
					...
					},
					{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_switch_right")
						},
						selects: [{
							type: "n_tab_lrhl",
							value: "s_right"
					...
					},
					{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_searchtxt_bg")
						},
						selects: [{
								type: "n_txtengine",
								value: "0"
					...
					},
					{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_reload_current")
						},
						selects: [{
							type: "n_tab",
							value: "s_current"
					...
					},
					{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_reload_clear")
						},
						selects: [{
							type: "n_tab",
							value: "s_current"
			...
			sdrg: {
				...
				lsdrg: [{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_openlnk_bg")
						},
						selects: [{
								type: "n_optype",
								value: "s_back"
				...
				isdrg: [{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_openimg_bg")
						},
						selects: [{
								type: "n_optype",
								value: "s_back"
			...
			drg: {
				...
				idrg: [{
						...
						mydes: {
							type: true,
							value: getDefault.i18n("init_searchimg_bg")
						},
						selects: [{
								type: "n_imgengine",
								value: "0"
...
var appConfmodel = {
	...
	homepage: {
		...
		sites: [
			[{
				title: "smartUp Gestures",
				url: "https://smartup.zimoapps.com/"
			}]
		],
		site: [{
			title: "Google",
	...
	extmgm: {
		...
		always: [] /*,group:[chrome.i18n.getMessage("extmgm_gplast"),chrome.i18n.getMessage("extmgm_gpalways")],exts:[]*/
	},
	notepad: {
...
var sub = {
		...
		if (config.general.fnswitch.fnicon) {
			...
			if (config.icon.settings.tip) {
				chrome.browserAction.setTitle({
					title: config.icon.actions[0].mydes && config.icon.actions[0].mydes.type && config.icon.actions[0].mydes.value ? config.icon.actions[0].mydes.value : sub.getI18n(config.icon.actions[0].name)
				});
			}
		} else {
			chrome.browserAction.setPopup({
		...
		chrome.contextMenus.removeAll(function () {
			...
			for (var i = 0; i < config.ctm.actions.length; i++) {
				var menuId = chrome.contextMenus.create({
					contexts: ["all"],
					title: ((config.ctm.actions[i].mydes && config.ctm.actions[i].mydes.type) ? config.ctm.actions[i].mydes.value : sub.getI18n(config.ctm.actions[i].name))
				}, function () {});
				sub.ctm.actions.push({
					id: i,
					menuId: menuId
				});
			}
				...
				var theFunction = function () {
					...
					chrome.topSites.get(function (sites) {
						...
						chrome.tabs.executeScript({
							code: 'chrome.runtime.sendMessage({type:"apps_test",apptype:"' + _appname + '",value:sue.apps.enable,ctm:true,appjs:appType["' + _appname + '"]},function(response){console.log(response)})',
		...
		switch (status) {
			...
			case "warning":
				chrome.browserAction.setIcon({
					tabId: tab.id,
					path: "../image/icon_warning.png"
				});
				chrome.browserAction.setTitle({
					tabId: tab.id,
					title: sub.getI18n("icon_tip")
				});
				break;
		}
	...
	action: {
		...
		close: function () {
			...
			let funClose = function () {
					...
					if (sub.curWin.tabs.length == ids.length && !sub.curWin.incognito && _closeKeep) {
						...
						chrome.tabs.create({}, function () {
							chrome.tabs.remove(ids, function () {
								(selvalue != "s_default") ? chrome.tabs.update(selid, {
									active: true
								}): null;
							});
						})
				...
				if (ids.length > 1 && sub.getConfValue("checks", "n_closeConfirm")) {
					chrome.tabs.sendMessage(sub.curTab.id, {
						type: "set_confirm"
					}, function (response) {
						if (response.message) {
							_funClose();
		...
		move: function () {
			...
			chrome.tabs.move(sub.curTab.id, {
				index: index
			});
		},
		detach: function () {
			...
			for (var i = 0; i < ids.length; i++) {
				chrome.tabs.duplicate(ids[i], function (tab) {
					if (sub.getConfValue("checks", "n_duplicatetype")) {
						chrome.tabs.update(sub.curTab.id, {
							highlighted: true
						});
					}
				});
		...
		mergewin: function () {
			sub.cons.mergewin = sub.cons.mergewin || {
				lastwins: []
			};
			let i = 0,
				ii = 0,
					...
					if (tabIds.length > 0 && windows.length > 1) {
						...
						for (i = 0; i < sub.cons.mergewin.lastwins.length; i++) {
							...
							(function (_temp, winId) {
								chrome.windows.create({
									tabId: sub.cons.mergewin.lastwins[_temp][0]
								}, function (window) {
									if (sub.cons.mergewin.lastwins[_temp].length > 1) {
										sub.cons.mergewin.lastwins[_temp].splice(0, 1);
										chrome.tabs.move(sub.cons.mergewin.lastwins[_temp], {
											windowId: window.id,
											index: -1
										}, function () {
		...
		mail: function () {
			...
			if (_mail == "s_defaultmail") {
				chrome.tabs.update(sub.curTab.id, {
					url: "mailto:?subject=" + _sub + '&body=' + _body
				});
			} else {
				chrome.tabs.create({
					url: "https://mail.google.com" + (_mail == "s_gmailapps" ? "/a/" + _domain : "") + "/mail/?view=cm&fs=1&tf=1&su=" + _sub + '&body=' + _body
				})
			}
		},
		print: function () {
		...
		recentht: function (call_appslist) {
			var theFunction = function () {
				...
				chrome.history.search({
					text: "",
					maxResults: parseInt(config.apps[_appname].n_num)
				}, function (items) {
					_obj.ht = items;
					sub.cons[_appname] = _obj;
					sub.insertTest(_appname);
		...
		apps_saveconf: function (message, sendResponse) {
			...
			sendResponse({
				...
				value: sub.getI18n("notif_con_appsave")
			})
		}
	},
			...
			for (i = 0; i < _type.length; i++) {
				for (ii = 0; ii < config[_type[i]].actions.length; ii++) {
					if (config[_type[i]].actions[ii].name == "close") {
						...
						config[_type[i]].actions[ii].checks.push({
							type: "n_closePin",
							value: false
						});
						config[_type[i]].actions[ii].checks.push({
			...
			for (var i = 0; i < 3; i++) {
				for (var ii = 0; ii < config.drg[arraydrg[i]].length; ii++) {
					if (config.drg[arraydrg[i]][ii].name == "txtsearch" ||
						...
						config.drg[arraydrg[i]][ii].selects.push({
							type: "n_encoding",
							value: "s_uri"
						});
					}
			...
			for (var i = 0; i < 3; i++) {
				for (var ii = 0; ii < config.sdrg[arraydrg[i]].length; ii++) {
					if (config.sdrg[arraydrg[i]][ii].name == "txtsearch" ||
						...
						config.sdrg[arraydrg[i]][ii].selects.push({
							type: "n_encoding",
							value: "s_uri"
						});
					}
		...
		f29tof30: function () {
			...
			for (var i = 0; i < config.drg.idrg.length; i++) {
				if (config.drg.idrg[i].name == "saveimg") {
					...
					config.drg.idrg[i].selects.push({
						type: "n_notif",
						value: "s_yes"
					});
				}
			...
			for (var i = 0; i < config.drg.ldrg.length; i++) {
				if (config.drg.ldrg[i].name == "bookmark") {
					config.drg.ldrg[i].selects.push({
						type: "n_notif",
						value: "s_yes"
					})
				}
			...
			for (var i = 0; i < config.sdrg.isdrg.length; i++) {
				if (config.sdrg.isdrg[i].name == "saveimg") {
					...
					config.sdrg.isdrg[i].selects.push({
						type: "n_notif",
						value: "s_yes"
					});
				}
			...
			for (var i = 0; i < config.sdrg.lsdrg.length; i++) {
				if (config.sdrg.lsdrg[i].name == "bookmark") {
					config.sdrg.lsdrg[i].selects.push({
						type: "n_notif",
						value: "s_yes"
					})
				}
	...
	funOnMessage: function (message, sender, sendResponse) {
		...
		switch (message.type) {
			...
			case "apps_test":
				...
				if (!message.value) {
					chrome.tabs.insertCSS({
						file: "css/apps_basic.css",
						runAt: "document_start"
					}, function () {})
					chrome.tabs.executeScript({
						file: "js/apps_basic.js",
						runAt: "document_start"
					}, function () {
						_fun();
					})
			...
			case "scroll":
				sendResponse({
					type: sub.cons.scroll.type,
					effect: sub.cons.scroll.effect
				});
				break;
			case "zoom":
				sendResponse({
	...
	apps: {
		autoreload: {
			...
			getConf: function (message, sender, sendResponse) {
				sendResponse({
					config: config.apps[message.app],
					value: sub.cons[message.app],
					tabId: sender.tab.id
				});
			}
		},
		...
		tablist: {
			...
			tabSwitch: function (message) {
				message.value ? chrome.tabs.update(Number(message.value), {
					active: true
				}) : null;
			}
		},
		...
		extmgm: {
			action: function (message, sender, sendResponse) {
				switch (message.value.actionType) {
					case "enable":
					case "disable":
						if (message.value.id == sub.extID || (config.apps.extmgm.always && config.apps.extmgm.always.contains(message.value.id))) {
							break
						}
			...
			disableAll: function (message, sender, sendResponse) {
				chrome.management.getAll(function (exts) {
					for (var i = 0; i < exts.length; i++) {
						if (exts[i].id == sub.extID || (config.apps.extmgm.always && config.apps.extmgm.always.contains(exts[i].id))) {
							continue;
						}
...
if (!chrome.runtime.getPlatformInfo) {} else {
...
if (!chrome.runtime.onInstalled) {} else {
			...
			chrome.storage.sync.get(function (items) {
				if (devMode || (items.general && items.general.settings.notif)) {
					var notif = {
						...
						items: [],
						buttons: [{
								title: sub.getI18n("notif_btn_open"),
								iconUrl: "image/open.svg"
							},
							{
...
if (!chrome.notifications) {} else {
	...
	chrome.notifications.onButtonClicked.addListener(function (id, index) {
		switch (index) {
			...
			case 1:
				chrome.tabs.create({
					url: "https://chrome.google.com/webstore/detail/" + chrome.runtime.id + "?hl=" + navigator.language
				});
				break;
		}
	})
