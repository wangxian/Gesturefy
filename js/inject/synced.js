...
sue.apps.synced = {
  ...
  initUI: function () {
    ...
    let _head = sue.apps.domCreate("div", {setName: ["className"], setValue: ["su_synced_head"]}),
      _spanSwitch = sue.apps.domCreate("span", {
        setName: ["className"],
        setValue: ["su_synced_switch"]
      }, null, null, null, sue.apps.i18n("app_synced_switch")),
    ...
    for (var i = 0; sue.apps.synced.sync.length > 0 && i < sue.apps.synced.sync[0].sessions.length; i++) {
      ...
      for (var ii = 0; ii < theobj.length; ii++) {
        var _li = sue.apps.domCreate("li", {setName: ["className"], setValue: ["su_synced_li"]}, null, null, {
          setName: ["id"],
          setValue: [theobj[ii].sessionId]
        }, theobj[ii].title);
        _ul.appendChild(_li);
  ...
  deviceChange: function (e) {
    ...
    for (var i = 0; i < sue.apps.synced.sync[sessionId].sessions.length; i++) {
      ...
      for (var ii = 0; ii < theobj.length; ii++) {
        var domli = sue.apps.domCreate("li", {setName: ["className"], setValue: ["su_synced_li"]}, null, null, {
          setName: ["id"],
          setValue: [theobj[ii].sessionId]
        }, theobj[ii].title);
