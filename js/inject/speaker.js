...
sue.apps.speaker = {
  ...
  initUI: function () {
    ...
    let _inputbox = sue.apps.domCreate("div", {setName: ["className"], setValue: ["speaker_inputbox"]}),
      ...
      _btnPlay = sue.apps.domCreate("button", {
        setName: ["className"],
        setValue: ["speaker_btn_play"]
      }, null, null, null, sue.apps.i18n("btn_play")),
      _btnPause = sue.apps.domCreate("button", {
        setName: ["className"],
        setValue: ["speaker_btn_pause"]
      }, null, "display:none;", null, sue.apps.i18n("btn_pause")),
      _btnStop = sue.apps.domCreate("button", {
        setName: ["className"],
        setValue: ["speaker_btn_stop"]
    ...
    switch (e.type) {
      case"click":
        if (e.target.className.indexOf("speaker_btn") == 0) {
          if (!sue.apps.getAPPboxEle(e).querySelector("textarea").value) {
            return;
          }
  ...
  speaker: function (e) {
    chrome.runtime.sendMessage({
      ...
      app: "speaker",
      action: "speak",
      value: {type: e.target.className.substr(12), txt: sue.apps.getAPPboxEle(e).querySelector("textarea").value}
    })
  }
}
