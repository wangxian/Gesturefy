  ...
  initUI: function () {
    ...
    for (var i = 0; i < this.list.length; i++) {
      ...
      var _li = sue.apps.domCreate("li", {
          setName: ["className"],
          setValue: ["su_tablist_li"]
        }, null, null, {
          setName: ["id"],
          setValue: [i]
        }, this.list[i].title),
        _img = sue.apps.domCreate("img"),
        _span = sue.apps.domCreate("span", {
          setName: ["className"],
          setValue: ["su_tablist_button_liclose"]
        }, null, null, null, "x");
      ...
      case "click":
        if (e.target.classList.contains("su_tablist_button_liclose")) {
          chrome.runtime.sendMessage({
            ...
            app: "tablist",
            action: "tabClose",
            value: sue.apps.tablist.list[e.target.parentNode.dataset.id].id
          });
        ...
        if (e.target.classList.contains("su_tablist_li") || (e.target.parentNode.classList.contains("su_tablist_li") && !e.target.classList.contains("su_tablist_button_liclose"))) {
          ...
          chrome.runtime.sendMessage({
            ...
            app: "tablist",
            action: "tabSwitch",
            value: sue.apps.tablist.list[theDom.dataset.id].id
          })
