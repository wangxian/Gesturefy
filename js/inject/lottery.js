...
sue.apps.lottery = {
  ...
  initUI: function () {
    ...
    let _typeLabel = sue.apps.domCreate("label", null, null, null, null, sue.apps.i18n("lottery_type")),
      _typeSelect = sue.apps.domCreate("select", {
        setName: ["className"],
        setValue: ["lottery_type"]
      }),
      _br0 = sue.apps.domCreate("br");
    _termLable = sue.apps.domCreate("label", null, null, null, null, sue.apps.i18n("lottery_term")),
      _termSelect = sue.apps.domCreate("select", {
        setName: ["className"],
        setValue: ["lottery_term"]
      }),
      _br1 = sue.apps.domCreate("br");
    _data = sue.apps.domCreate("div", {
      setName: ["className"],
      setValue: ["lottery_data"]
    });
    let typeArray = ["lottery_dlt", "lottery_pls", "lottery_ssq", "lottery_sd"];
    for (var i = 0; i < typeArray.length; i++) {
      _typeSelect.appendChild(sue.apps.domCreate("option", {
        setName: ["value"],
        setValue: [typeArray[i]]
      }, null, null, null, sue.apps.i18n(typeArray[i])));
    }
    ...
    switch (e.type) {
      case "change":
        chrome.runtime.sendMessage({
          type: "appsAction",
          app: "lottery",
          action: (e.target.classList.contains("lottery_type") ? "getTerm" : "getData"),
          value: {
            type: sue.apps.lottery.dom.querySelector(".lottery_type").value,
            term: sue.apps.lottery.dom.querySelector(".lottery_term").value
          }
        });
        sue.apps.lottery.loading();
        break;
  ...
  term: function (message) {
    ...
    _box.textContent = "";
    for (var i = 0; i < message.value.length; i++) {
      _box.appendChild(sue.apps.domCreate("option", {
        setName: ["value"],
        setValue: [message.value[i]]
      }, null, null, null, message.value[i]));
    }
    chrome.runtime.sendMessage({
      type: "appsAction",
      app: "lottery",
  ...
  loading: function () {
    let _box = sue.apps.lottery.dom.querySelector(".lottery_data");
    _box.textContent = "";
    _box.appendChild(sue.apps.domCreate("img", {
      setName: ["src"],
