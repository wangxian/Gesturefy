...
sue.apps.rss = {
  ...
  initUI: function () {
    ...
    let appInfo = {
      ...
      menu: [{
        src: "/image/menu.svg",
        title: "app_rss_menu",
        className: "menu_item menu_item_rss"
      },
        {
    ...
    //rss head
    let _rssHead = sue.apps.domCreate("div", {
      setName: ["className"],
      setValue: ["rss_head"]
    });
    let _rssTitle = sue.apps.domCreate("span", {
      setName: ["className"],
      setValue: ["su_rss_rsshead"]
    });
    _rssHead.appendChild(_rssTitle);
    dom.querySelector(".su_main").appendChild(_rssHead);
    //rss box
    let _rssBox = sue.apps.domCreate("div", {
      setName: ["className"],
      setValue: ["rss_box"]
    });
    ...
    //sub box
    let _rssSub = sue.apps.domCreate("div", {
      setName: ["className"],
      setValue: ["rss_sub"]
    });
    let _subBox = sue.apps.domCreate("div", {
      setName: ["className"],
      setValue: ["sub_box"]
    });
    let _subText = sue.apps.domCreate("input", {
      setName: ["className"],
      setValue: ["sub_text"]
    });
    _subText.type = "text";
    _subText.placeholder = sue.apps.i18n("app_rss_subplace");
    let _subBtn = sue.apps.domCreate("input", {
      setName: ["className"],
      setValue: ["sub_add"]
    });
    _subBtn.type = "button";
    ...
    let _subItem = sue.apps.domCreate("ul", {
      setName: ["className"],
      setValue: ["sub_items"]
    });
  ...
  rssAdd: function (e) {
    ...
    var domli = sue.apps.domCreate("li", {
      setName: ["className", "title"],
      setValue: ["sub_item", theurl]
    }, null, null, {
      setName: ["url", "id"],
      setValue: [theurl, sue.apps.rss.config.feed.length]
    }, theurl);
    var domdel = sue.apps.domCreate("span", {
      setName: ["className"],
      setValue: ["sub_itemdel"]
    }, null, null, null, "x");
  ...
  menu: function (dom) {
    ...
    chrome.storage.local.get(function (items) {
      ...
      for (var i = 0; i < feed.length; i++) {
        var domli = sue.apps.domCreate("li", {
          setName: ["className", "title"],
          setValue: ["sub_item", feed[i]]
        }, null, null, {
          setName: ["url", "id"],
          setValue: [feed[i], i]
        }, feedtitle[i] ? feedtitle[i] : feed[i]);
        var domdel = sue.apps.domCreate("span", {
          setName: ["className"],
          setValue: ["sub_itemdel"]
        }, null, null, null, "x");
